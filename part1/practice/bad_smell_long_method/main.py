...
def _filter(data):
    return [x for x in data if int(x[1]) > SORT_YEARS_LIMIT]

def get_users_list(csv_data):
    data = _read(csv_data=csv)
    data = _sort(data=data)
    return _filter(data=data)
...
if __name__ == '__main__':
    ...
    #print(_filter(_sort(_read(csv_data=csv))))
